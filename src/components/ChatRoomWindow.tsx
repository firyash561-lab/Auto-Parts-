...
export default function ChatRoomWindow({ chat, currentUser, onClose }: ChatRoomWindowProps) {
  ...
  return (
    <div className="absolute inset-0 bg-slate-50 flex flex-col z-40" id={`chat-room-${chat.id}`}>
      {/* Top Header */}
      <div className="bg-[#0B1220] text-white pt-4 pb-3.5 px-4 flex items-center gap-3 shadow-md border-b border-[#18233C]">
        <button 
          onClick={onClose}
        ...
        {/* Small avatar */}
        <div className="w-8.5 h-8.5 bg-[#2563EB]/25 text-[#60A5FA] border border-[#2563EB]/30 rounded-full flex items-center justify-center font-bold text-xs uppercase shrink-0">
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg) => {
            ...
            return (
              ...
              >
                <div 
                  className={`p-3 rounded-2xl text-xs leading-relaxed font-medium shadow-xs break-words ${
                    isMe 
